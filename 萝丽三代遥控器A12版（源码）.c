...
//蜂鸣器
bit Message_AlarmClock,Message_LowPower;
...
bit throttle_lock,Throttle_check=1,Auto_lock,Trim_step;
u8 tt=0;
...
int data PPM_buff[8];
u16 data PPMout[8]={500,600,800,200,1000,0,100,300};
...
{								
	...
	sum_read=EEPROM_read(0,74);//读校验和
	sum_read<<=8;
	sum_read+=EEPROM_read(0,75);
...
//		EEPROM_write(0,55,auto_mapping);
	...
	EEPROM_write(0,74,sum>>8);
	EEPROM_write(0,75,sum);	
